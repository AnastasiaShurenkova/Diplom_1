...
class TestBurger:
    ...
    def test_move_ingridient(self):
        ...
        new_first_index = new_receipt.find(first_ingridient_name)
        new_second_index = new_receipt.find(second_ingridient_name)

        assert first_index < second_index and new_first_index > new_second_index
    ...
    def test_remove_ingridient(self):
        ...
        assert first_ingridient.get_name() in receipt and second_ingridient.get_name() in receipt and first_ingridient.get_name() not in new_receipt and second_ingridient.get_name() in new_receipt
    ...
    def test_print_receipt(self, mocked_print):
        ...
        assert mocked_print.mock_calls == [call(receipt), call()]
    ...
    def test_get_price(self):
        ...
        burger = Burger()
        burger.set_buns(bun)
        bun_price = 2 * bun.get_price()
        burger.add_ingredient(ingridient)
        bun_price += ingridient.get_price()
        receipt_price = burger.get_price()

        assert receipt_price == bun_price